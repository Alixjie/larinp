...
struct multiboot_t {
	...
	uint_t boot_device;		// 指出引导程序从哪个BIOS磁盘设备载入的OS映像
	uint_t cmdline;		// 内核命令行
	uint_t mods_count;		// boot 模块列表
	uint_t mods_addr;
	...
	uint_t drives_length; 	// 指出第一个驱动器结构的物理地址	
	uint_t drives_addr; 		// 指出第一个驱动器这个结构的大小
	uint_t config_table; 		// ROM 配置表
	uint_t boot_loader_name; 	// boot loader 的名字
	uint_t apm_table; 	    	// APM 表
	uint_t vbe_control_info;
	uint_t vbe_mode_info;
	uint_t vbe_mode;
	uint_t vbe_interface_seg;
	uint_t vbe_interface_off;
	uint_t vbe_interface_len;
...
typedef
struct mmap_entry_t {
	uint_t size; 		// 留意 size 是不含 size 自身变量的大小
	uint_t base_addr_low;
	uint_t base_addr_high;
	uint_t length_low;
	uint_t length_high;
	uint_t type;
